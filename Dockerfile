...
RUN mkdir -p /squads-public-v3-build/dist && \
    mv dist/* /squads-public-v3-build/dist && \
    cd /squads-public-v3-build/dist && \
    find . -type f -print0 | sort -z | xargs -0 cat | sha256sum | awk '{ print $1 }' > /squads-public-v3-build/hash.txt

# Set proper permissions
RUN chmod -R u+rwX,go+rX /squads-public-v3-build/dist && \
    chmod u+rw,go+r /squads-public-v3-build/hash.txt

# Copy the build output to a standard location
RUN mkdir -p /output && cp -r /squads-public-v3-build/* /output/
